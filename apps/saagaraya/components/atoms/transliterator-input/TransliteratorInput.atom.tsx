...
type TransliteratorInputProps = TextInputProps & {
  enableTransliteration: boolean;
};

export function TransliteratorInput({
  enableTransliteration,
  placeholder,
  error,
  onChange,
  ...props
}: TransliteratorInputProps) {
  ...
  return (
    <div className="flex w-full flex-col overflow-hidden">
      <div className="relative">
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex h-9 w-full items-center px-[0.75rem] text-sm">
          <span className="invisible whitespace-pre text-black">{text}</span>
          {textToTransliterate && (
        ...
        <span className="flex flex-col text-neutral-800 dark:text-neutral-200">
          <span className="flex flex-row-reverse items-center gap-1 text-sm">
            {props.maxLength && totalLength >= props.maxLength && (
              <span className="rounded-full px-2 py-0.5 text-xs font-semibold text-white dark:bg-red-600">
                උපරිම
              </span>
            )}
            {transliteratedText}
